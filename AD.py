...
if __name__ == "__main__":
    ...
    for portInfo in callback.getDetectedDots():
        ...
        device.setLogOptions(xsensdot_pc_sdk.XsLogOptions_Euler)
    ...
    startTime = xsensdot_pc_sdk.XsTimeStamp_nowMs()
    while xsensdot_pc_sdk.XsTimeStamp_nowMs() - startTime <= 60000 and not exit_flag:
        if exit_flag:
            break
        if callback.packetsAvailable():
            ...
            if not orientationResetDone and xsensdot_pc_sdk.XsTimeStamp_nowMs() - startTime > 30000:
                for device in deviceList:
